...
class Program
{
    ...
    function insertProgram($programArr, $providerUuid)
    {
        ...
        $programUuid = id_q("select uuid from program where name = '{$name}'");
        if (empty($programUuid)) {
            ...
            $programUuid = $programUuid['uuid'];
            $programQuery = "update program set provider_uuid = {$providerUuid}, name = '{$name}', budget = {$budget} where uuid = {$programUuid}";
            action_q($programQuery);
        }
